...
def build_dataset(is_train, config):
    ...
    if config.DATA.DATASET == 'imagenet':
        ...
        dataset = torchvision.datasets.CIFAR100(config.DATA.DATA_PATH, train=is_train, download=False, transform=transform)
        nb_classes = config.MODEL.NUM_CLASSES
    elif config.DATA.DATASET == 'CIFAR10':
        dataset = torchvision.datasets.CIFAR10(config.DATA.DATA_PATH, train=is_train, download=False, transform=transform)
        nb_classes = config.MODEL.NUM_CLASSES
    elif config.DATA.DATASET == '101_ObjectCategories':
        prefix = 'train' if is_train else 'val'
        root = os.path.join(config.DATA.DATA_PATH, prefix)
        dataset = datasets.ImageFolder(root, transform=transform)
        nb_classes = config.MODEL.NUM_CLASSES
    elif config.DATA.DATASET == 'imagenet-1k':
        prefix = 'train' if is_train else 'val'
        root = os.path.join(config.DATA.DATA_PATH, prefix)
        dataset = datasets.ImageFolder(root, transform=transform)
        nb_classes = config.MODEL.NUM_CLASSES
